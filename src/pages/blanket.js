...
export default function GM(props) {
  ...
  React.useEffect(async () => {
    ...
    //const model = await tf.loadLayersModel('/blanket/model.json');
    ...
    const imageTensor = tf.browser.fromPixels(imageElement).resizeBilinear([224,224]);  // for example
    const expandedImageTensor = tf.expandDims(imageTensor, 0); ;
    const probs = model.predict(expandedImageTensor);
    console.log(probs);
    alert(probs);
    const sorted = true;
